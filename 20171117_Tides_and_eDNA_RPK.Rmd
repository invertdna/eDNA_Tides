...
tidedir<-"/Users/rpk/GoogleDrive/Kelly_Lab/Projects/TideEffectHoodCanal_Gallego/"
#tidedir<-"/Users/rgallego/Google Drive File Stream/My Drive/Kelly_Lab/Projects/TideEffectHoodCanal_Gallego/"
setwd(paste0(tidedir,"Analysis"))
Data_dir<-paste0(tidedir,"Data")
#How to check if you have all needed packages installed, introducing: uptodate.R
source(paste0(tidedir,"uptodate.R"))
libs=c("vegan","plotrix","MASS","data.table", "gdata","lattice","plyr","dplyr", "arm", "gridExtra", "ggplot2", "eeptools", "taxize", "lmtest","readxl", "Biostrings", "googlesheets", "tidyr", "ggmap", "xtable", "tibble","pander","RColorBrewer", "ggfittext", "treemapify", "kableExtra", "knitr")
uptodate(libs)
#lapply(libs, require, character.only = TRUE)

source("multiplot.R") #ggplot multiplot function from file
...
#MiSeq full run metadata
  	meta=read.csv(paste0(Data_dir, "/COI/FullRun/metadata_wTides.csv"))
  	sampleInfo<-as.data.frame(gs_read(gs_title("Step1_Samples"))) #field data for covariates; read from googlesheets
...
###get metadata from nano run, and merge
    metaNano=read.csv(paste0(Data_dir,"/COI/Nano_banzai_out_20170719_1420/metadata_correctedRamon_20170926.csv"))
...
#OTU data
allOTUs<-
	read.table(paste0(Data_dir, "/COI/CombinedNanoFullRun/OTUs_swarm/OTU.map")) %>%
...
load("OccupancyModeling/ProbOccCOI_dataOUT08_22_46.Rdata")
...
#for cumulative probability of individual Families	  
    load("ShinyApp_Occupancy_HoodCanal/OTUprob.RData")
...
tax<-read.csv("blasting/masterDNAtable_20171012.csv", header=T, row.names=1)
...
NOAA<-read.table(paste0(Data_dir,"/NOAAtideData_UnionMarch2017.txt"), skip=13, header=T)
...
plotmap + 
	geom_point(data=data.frame(lats,longs), aes(x=longs, y=lats), color="red", size=4) +
...
```{r create_sampling_table, echo=F}
#xtabs(~Sites+Tides)
...
[X% of OTUs could be annotated at Y level]
...
##Create distance matrices. Counts will use Bray-Curtis; occupancy probabilities will use Bray or?

OTU.count.distance<-as.matrix(vegdist(t(rrOTUs), method="bray", diag=F, upper=F, binary=F))
OTU.prob.distance<-
  OTUprob[,c("OTUname","bottle","probOcc")]%>%
  spread(key=bottle, value=probOcc)%>%
  Col2RN(1)%>%
  t()%>%
...
Family.prob.distance<-
  aggregate(OTUprob$probOcc, list(OTUprob$OTU_taxon_family, OTUprob$bottle), FUN=cumProb) %>%
...
#print(xtable(OTU.count.adonis))  #use this to print table of Adonis results; otherwise, use the code in the next chunk to plot chart.

#adonis(Family.count.distance~rrMeta$Tide+rrMeta$SamplingEvent+rrMeta$original_sample)
#adonis(Family.prob.distance~rrMeta$Tide[match(colnames(Family.prob.distance),     rrMeta$original_sample)]+rrMeta$SamplingEvent[match(colnames(Family.prob.distance), rrMeta$original_sample)])
#adonis(OTU.prob.distance~rrMeta$Tide[match(colnames(OTU.prob.distance), rrMeta$original_sample)]+rrMeta$SamplingEvent[match(colnames(OTU.prob.distance), rrMeta$original_sample)])
...
##Do the appropriate samples/replicates cluster together?  2 views: heatmap and CCA
...
```{r multiplot_NMDS_BottleTide, fig.width=6, fig.height=7.2, fig.path='figures/', dev=c('png', 'pdf'), echo=F, fig.cap="\\label{fig:fig3}(A) Ordination plot (non-metric multidimensional scaling; NMDS) plot of Bray-Curtis dissimilarities among sequenced replicates, by sampling bottle (polygon) and tide (polygon color). (B) The same data shown as a heatmap. The outlying Twanoh samples stand out as the darker portions in the upper-right."}     	
fig3a<-      ggplot() + 
      ...
      fig3b<-  ggplot(distmat, aes(Var1, Var2, fill=value))+
          geom_tile()+
...
```{r tide_community_figure, fig.path='figures/', dev=c('png', 'pdf'), echo=F,echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, fig.cap="\\label{fig:fig4}eDNA Communities by Time, Tide, and Site."}


#plot w ggplot
    f<-ggplot(data=NOAA[130:350,], aes(x=DateTime, y=Pred))+
      ylab("Tidal Height (m)")+
      xlab("Day and Time")
    f+geom_smooth(se=F, span=.22, na.rm=F, color="grey")+
...
kable(as.data.frame(anova(
  glm(Community~as.numeric(Salinity_refractometer)*as.numeric(Temperature), data=rrMeta, family="binomial", na.action = na.omit)
                          )), format="latex", align="c", caption="The salinity and temperature of the sampled water mass explain nearly all of the variation in community identity across sites")
...
```{r TW_Family_turnover, fig.path='figures/', dev=c('png', 'pdf'), echo=F, warning=F, message=F}
FamilyCounts<-  
  aggregate.data.frame(rrOTUs, list(tax$OTU_taxon_family[match(row.names(rrOTUs), row.names(tax))]), FUN=sum) %>%
  Col2RN(1)
...
FamilyCumProb<-
  dataOUT%>%
  ddply(c("bottle", "OTU_taxon_family"), summarize, CumProb=cumProb(probOcc))
FamilyCumProb[,4:13]<-dataOUT[match(FamilyCumProb$bottle, dataOUT$bottle),c(26,28:35)]


#Heatmap by Cumulative Probability
TW.FamilyCumProb<-subset(FamilyCumProb, FamilyCumProb$site=="TW")
  TW.FamilyCumProb<-TW.FamilyCumProb[!is.na(TW.FamilyCumProb$OTU_taxon_family),]
  TW.FamilyCumProb$bottle<-drop.levels(TW.FamilyCumProb$bottle)
    v<-spread(TW.FamilyCumProb[,1:3], key=OTU_taxon_family, value=CumProb) #use for ordering families by probability
  TW.FamilyCumProb$OTU_taxon_family <- factor(TW.FamilyCumProb$OTU_taxon_family, levels=names(sort(colSums(v[,2:ncol(v)]))))
    
            # (p <- ggplot(TW.FamilyCumProb, aes(bottle, OTU_taxon_family)) + 
            #     geom_tile(aes(fill = CumProb), colour = "white") +
...
TW.FamilyCountsSums<-FamilyCountsSums[,grepl("TW", colnames(FamilyCountsSums))]
  TW.FamilyCountsSums<-TW.FamilyCountsSums[rowSums(TW.FamilyCountsSums)>20,]  #only show more common things?
    SortedFams<-names(sort(rowSums(TW.FamilyCountsSums), decreasing = F)) #ordering by count
    TW.FamilyCountsSums<-melt(TW.FamilyCountsSums); colnames(TW.FamilyCountsSums)<-c("Family","Bottle","CumulativeReadCount")
    TW.FamilyCountsSums$Family<-factor(TW.FamilyCountsSums$Family, levels=SortedFams)
    
    (q <- ggplot(TW.FamilyCountsSums, aes(Bottle, Family)) + 
        geom_tile(aes(fill = log(CumulativeReadCount)), colour = "white") +
        geom_vline(aes(xintercept=6.5),colour="black")+
        geom_vline(aes(xintercept=9.5),colour="black")+
        scale_fill_gradient(low = "white",high = "steelblue")+
        theme(axis.text.x=element_text(angle=90, hjust=1))+
        guides(fill=guide_legend(title="Log(Reads)"))
    )
