...
```{r multiplot_community_membership, fig.width=5, fig.height=6, fig.path='figures/', dev=c('png', 'pdf'), echo=F,fig.cap="\\label{fig:fig5}Tidal height (m), Water temperature (C), DNA concentration recovered (ng/mL), Salinity (ppt), and proportion of DNA reads allocated among the top 10 Families in the annotated dataset."}
  #################
    NOAA2<-
      NOAA%>%
...
s5<-ggplot(z, aes(y=value, x=variable, fill=Family))+
    ...
    guides(fill=guide_legend(ncol=5), size=2)+xlab("Sample")+ylab("Proportion Reads, Top 10 Families")+theme(plot.margin = unit(c(0,3,0,0), "pt"))
